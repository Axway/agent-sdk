...
type MetricReportingServiceClient interface {
	...
	MetricReporting(ctx context.Context, in *MetricServiceInit, opts ...grpc.CallOption) (grpc.ServerStreamingClient[MetricReport], error)
}
...
func (c *metricReportingServiceClient) MetricReporting(ctx context.Context, in *MetricServiceInit, opts ...grpc.CallOption) (grpc.ServerStreamingClient[MetricReport], error) {
...
type MetricReportingService_MetricReportingClient = grpc.ServerStreamingClient[MetricReport]
...
type MetricReportingServiceServer interface {
	// The agent will initiate the connection to the service and the service can then begin to send metric data for the SDK to handle
	MetricReporting(*MetricServiceInit, grpc.ServerStreamingServer[MetricReport]) error
	mustEmbedUnimplementedMetricReportingServiceServer()
}

// UnimplementedMetricReportingServiceServer must be embedded to have
...
func (UnimplementedMetricReportingServiceServer) MetricReporting(*MetricServiceInit, grpc.ServerStreamingServer[MetricReport]) error {
	return status.Error(codes.Unimplemented, "method MetricReporting not implemented")
}
func (UnimplementedMetricReportingServiceServer) mustEmbedUnimplementedMetricReportingServiceServer() {
}
func (UnimplementedMetricReportingServiceServer) testEmbeddedByValue() {}
...
func RegisterMetricReportingServiceServer(s grpc.ServiceRegistrar, srv MetricReportingServiceServer) {
	// If the following call panics, it indicates UnimplementedMetricReportingServiceServer was
...
func _MetricReportingService_MetricReporting_Handler(srv interface{}, stream grpc.ServerStream) error {
	...
	return srv.(MetricReportingServiceServer).MetricReporting(m, &grpc.GenericServerStream[MetricServiceInit, MetricReport]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type MetricReportingService_MetricReportingServer = grpc.ServerStreamingServer[MetricReport]
...
// UnimplementedQuotaEnforcementServer must be embedded to have
// forward compatible implementations.
//
// NOTE: this should be embedded by value instead of pointer to avoid a nil
// pointer dereference when methods are called.
...
func (UnimplementedQuotaEnforcementServer) QuotaEnforcementInfo(context.Context, *QuotaInfo) (*QuotaEnforcementResponse, error) {
	return nil, status.Error(codes.Unimplemented, "method QuotaEnforcementInfo not implemented")
}
func (UnimplementedQuotaEnforcementServer) mustEmbedUnimplementedQuotaEnforcementServer() {}
func (UnimplementedQuotaEnforcementServer) testEmbeddedByValue()                          {}
...
func RegisterQuotaEnforcementServer(s grpc.ServiceRegistrar, srv QuotaEnforcementServer) {
	// If the following call panics, it indicates UnimplementedQuotaEnforcementServer was
	// embedded by pointer and is nil.  This will cause panics if an
	// unimplemented method is ever invoked, so we test this at initialization
	// time to prevent it from happening at runtime later due to I/O.
	if t, ok := srv.(interface{ testEmbeddedByValue() }); ok {
