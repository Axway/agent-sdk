...
// TestEventListener_kindLane proves the two properties the shard-by-resource-ID design depends on:
// the same resource ID always lands on the same shard (so its events stay ordered relative to each
// other, same as when a kind had only one worker), and different resource IDs spread across more
// than one shard (so different resources stop contending for the same worker).
func TestEventListener_kindLane(t *testing.T) {
	const kind = "TestKind"
	em := &EventListener{kindJobs: map[string][]chan handlerData{kind: make([]chan handlerData, 8)}}
	for i := range em.kindJobs[kind] {
		em.kindJobs[kind][i] = make(chan handlerData, 1)
	}

	ch, label := em.kindLane(kind, "resource-a")
	for i := 0; i < 5; i++ {
		gotCh, gotLabel := em.kindLane(kind, "resource-a")
		assert.True(t, ch == gotCh, "the same resource ID must always land on the same shard")
		assert.Equal(t, label, gotLabel)
	}

	seen := map[string]bool{}
	for i := 0; i < 50; i++ {
		_, label := em.kindLane(kind, fmt.Sprintf("resource-%d", i))
		seen[label] = true
	}
	assert.Greater(t, len(seen), 1, "expected different resource IDs to spread across more than one shard")

	gotCh, gotLabel := em.kindLane("UnknownKind", "resource-a")
	assert.Nil(t, gotCh, "a kind with no registered lane should return no channel instead of blocking")
	assert.Equal(t, regularQueue, gotLabel)
}

// TestEventListener_kindShards_runConcurrently proves that different resources within the same
// kind run on different shards concurrently - the reason kindJobs was sharded instead of N workers
// pulling from one shared channel per kind.
func TestEventListener_kindShards_runConcurrently(t *testing.T) {
	slow := &slowHandler{delay: 200 * time.Millisecond}
...
// concurrent-map-write crash (SetSubResource racing CreateHashes on the shared SubResources map)
// and silent out-of-order overwrites for a single resource - while different resources are still
// free to run in parallel across the other workers.
