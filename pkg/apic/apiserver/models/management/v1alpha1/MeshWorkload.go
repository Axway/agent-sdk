...
func NewMeshWorkload(name, scopeKind, scopeName string) (*MeshWorkload, error) {
	validScope := false
	for _, s := range MeshWorkloadScopes {
		if scopeKind == s {
			validScope = true
			break
		}
	...
	if !validScope {
		return nil, fmt.Errorf("scope '%s' not valid for MeshWorkload kind", scopeKind)
	}
