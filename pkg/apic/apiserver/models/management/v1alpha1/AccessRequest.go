...
func NewAccessRequest(name, scopeKind, scopeName string) (*AccessRequest, error) {
	validScope := false
	for _, s := range AccessRequestScopes {
		if scopeKind == s {
			validScope = true
			break
		}
	...
	if !validScope {
		return nil, fmt.Errorf("scope '%s' not valid for AccessRequest kind", scopeKind)
	}
