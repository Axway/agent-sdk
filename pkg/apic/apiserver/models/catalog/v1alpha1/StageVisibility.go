/*
 * This file is automatically generated
 */

package catalog

import (
	"encoding/json"

	apiv1 "github.com/Axway/agent-sdk/pkg/apic/apiserver/models/api/v1"

	"github.com/Axway/agent-sdk/pkg/util/log"
)

var (
	StageVisibilityCtx log.ContextField = "stageVisibility"

	_StageVisibilityGVK = apiv1.GroupVersionKind{
		GroupKind: apiv1.GroupKind{
			Group: "catalog",
			Kind:  "StageVisibility",
		},
		APIVersion: "v1alpha1",
	}

	StageVisibilityScopes = []string{"Marketplace"}
)

const (
	StageVisibilityResourceName             = "stagevisibility"
	StageVisibility_embeddedSubResourceName = "_embedded"
)

func StageVisibilityGVK() apiv1.GroupVersionKind {
	return _StageVisibilityGVK
}

func init() {
	apiv1.RegisterGVK(_StageVisibilityGVK, StageVisibilityScopes[0], StageVisibilityResourceName)
	log.RegisterContextField(StageVisibilityCtx)
}

// StageVisibility Resource
type StageVisibility struct {
	apiv1.ResourceMeta
	_embedded interface{}         `json:"_embedded"`
	Owner     *apiv1.Owner        `json:"owner"`
	Spec      StageVisibilitySpec `json:"spec"`
}

// NewStageVisibility creates an empty *StageVisibility
func NewStageVisibility(name, scopeName string) *StageVisibility {
	return &StageVisibility{
		ResourceMeta: apiv1.ResourceMeta{
			Name:             name,
			GroupVersionKind: _StageVisibilityGVK,
			Metadata: apiv1.Metadata{
				Scope: apiv1.MetadataScope{
					Name: scopeName,
					Kind: StageVisibilityScopes[0],
				},
			},
		},
	}
}

// StageVisibilityFromInstanceArray converts a []*ResourceInstance to a []*StageVisibility
func StageVisibilityFromInstanceArray(fromArray []*apiv1.ResourceInstance) ([]*StageVisibility, error) {
	newArray := make([]*StageVisibility, 0)
	for _, item := range fromArray {
		res := &StageVisibility{}
		err := res.FromInstance(item)
		if err != nil {
			return make([]*StageVisibility, 0), err
		}
		newArray = append(newArray, res)
	}

	return newArray, nil
}

// AsInstance converts a StageVisibility to a ResourceInstance
func (res *StageVisibility) AsInstance() (*apiv1.ResourceInstance, error) {
	meta := res.ResourceMeta
	meta.GroupVersionKind = StageVisibilityGVK()
	res.ResourceMeta = meta

	m, err := json.Marshal(res)
	if err != nil {
		return nil, err
	}

	instance := apiv1.ResourceInstance{}
	err = json.Unmarshal(m, &instance)
	if err != nil {
		return nil, err
	}

	return &instance, nil
}

// FromInstance converts a ResourceInstance to a StageVisibility
func (res *StageVisibility) FromInstance(ri *apiv1.ResourceInstance) error {
	if ri == nil {
		res = nil
		return nil
	}
	var err error
	rawResource := ri.GetRawResource()
	if rawResource == nil {
		rawResource, err = json.Marshal(ri)
		if err != nil {
			return err
		}
	}
	err = json.Unmarshal(rawResource, res)
	return err
}

// MarshalJSON custom marshaller to handle sub resources
func (res *StageVisibility) MarshalJSON() ([]byte, error) {
	m, err := json.Marshal(&res.ResourceMeta)
	if err != nil {
		return nil, err
	}

	var out map[string]interface{}
	err = json.Unmarshal(m, &out)
	if err != nil {
		return nil, err
	}

	out["_embedded"] = res._embedded
	out["owner"] = res.Owner
	out["spec"] = res.Spec

	return json.Marshal(out)
}

// UnmarshalJSON custom unmarshaller to handle sub resources
func (res *StageVisibility) UnmarshalJSON(data []byte) error {
	var err error

	aux := &apiv1.ResourceInstance{}
	err = json.Unmarshal(data, aux)
	if err != nil {
		return err
	}

	res.ResourceMeta = aux.ResourceMeta
	res.Owner = aux.Owner

	// ResourceInstance holds the spec as a map[string]interface{}.
	// Convert it to bytes, then convert to the spec type for the resource.
	sr, err := json.Marshal(aux.Spec)
	if err != nil {
		return err
	}

	err = json.Unmarshal(sr, &res.Spec)
	if err != nil {
		return err
	}

	// marshalling subresource _embedded
	if v, ok := aux.SubResources["_embedded"]; ok {
		sr, err = json.Marshal(v)
		if err != nil {
			return err
		}

		delete(aux.SubResources, "_embedded")
		err = json.Unmarshal(sr, &res._embedded)
		if err != nil {
			return err
		}
	}

	return nil
}

// PluralName returns the plural name of the resource
func (res *StageVisibility) PluralName() string {
	return StageVisibilityResourceName
}
