...
func TestAPIService(t *testing.T) {
	...
	// Create an API Service scoped to the environment above
	...
	// Update the created API Service
	svc.Spec.Description = "updated description"
	_, err = svcClient.Update(svc)
	if err != nil {
		t.Fatalf("Failed to update: %s", err)
	}

	// Get single API Service by name
	_, err = svcClient.Get(svc.Name)
	if err != nil {
		t.Fatalf("Failed to get %s by name: %s", svc.Name, err)
	}
