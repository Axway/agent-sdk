...
func (c *cachePubSub) Publish(data interface{}) error {
	changed, err := globalCache.HasItemChanged(c.topic, data)
	if err != nil {
		return fmt.Errorf("Could not check if the data changed %s", err.Error())
	}
	if !changed {
		return nil
	}
